...
void startup() // 数据初始化
{
	...
	mciSendString("open bj.mp3 alias bkmusic", NULL, 0, NULL);   //背景音乐
	mciSendString("play bkmusic repeat", NULL, 0, NULL);    //循环播放
	initgraph(Width, High);
	loadimage(&img_human2,".\\图片素材\\马里奥.jpg");
	loadimage(&img_bk,".\\图片素材\\背景.jpg");
	loadimage(&img_human1,".\\图片素材\\马里奥掩码图.jpg");
	loadimage(&img_pd2,".\\图片素材\\炮弹.jpg");
	loadimage(&img_pd1,".\\图片素材\\炮弹掩码.jpg");
	loadimage(&img_gs1,".\\图片素材\\野怪2 掩码图.jpg");
	loadimage(&img_gs2,".\\图片素材\\野怪2.jpg");
	loadimage(&img_ks,".\\图片素材\\ks.jpg");	
	BeginBatchDraw();
	while(gameStatus==0)
		startMenu();
...
void paodan(){
	...
	for(i=0;i<3;i++)
	{
		if((abs(human_x+rwWidth/4-zpt[i].pd_x)<=5)&&(abs(human_y-zpt[i].pd_y)<=40))
		{
			...
			mciSendString("open 死亡.mp3 alias swmusic", NULL, 0, NULL);
...
void fantan(){
	...
	if(x>=fzk[0].qs_x&&x<=fzk[0].zd_x&&human_y>=fzk[0].high-30&&human_y<=fzk[0].high)
	{
		...
		if(fzk[0].zjcs==1)
		{
			...
			mciSendString("open 顶砖块.mp3 alias zkmusic", NULL, 0, NULL);
...
void updateWithoutInput()  // 与用户输入无关的更新
{
	...
	if(x>=9910) //通关成功 
	{
		...
		mciSendString("open lq.mp3 alias lqmusic", NULL, 0, NULL);
	...
	void updateWithInput()  // 与用户输入有关的更新
	{
		...
		if((GetAsyncKeyState(VK_SPACE)&0x8000)&&sfkcz==1)  //空格 
		{
			if(pdtg==0&&jiao_y>=436&&human_y<456||pdtg==0&&sfkt==1)
			{
				pdtg=1;
				mciSendString("close jpmusic", NULL, 0, NULL);
				mciSendString("open 跳跃.mp3 alias jpmusic", NULL, 0, NULL);
